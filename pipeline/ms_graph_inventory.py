...
def init_clients_creds():
    ...
    secret_id = auth_map["secret_id"] #can facilitate tracking auth issues by id in the app 
   
    logger.info(f'setting graph api client: {tenant_id}, clientid(app_id): {tenant_id}, secret id: {secret_id}')
    creds=ClientSecretCredential(tenant_id=tenant_id,client_id=client_id,client_secret=app_secret)
...
def graph_generator_f(graph_client,endpoint=None, max_page=3):
    ...
    while endpoint and (max_page==0 or page_num<=max_page):
        ...
        logger.info(f'collected items: {len(page_records_batch)}, cumulative count {cumulative_item_count}')
        yield from page_records_batch
        endpoint = response_raw["@odata.nextLink"] if "@odata.nextLink" in response_raw else None
...
def make_base_url(endpoint,page_size=500,extra_args=None):
    ...
    first_args_separator="" if ( page_size_str=="" and extra_args_str=="" ) else "?"
    next_arg_separator="&" if ( page_size_str!="" and extra_args_str!="" ) else ""
    url=f'{endpoint}{first_args_separator}{page_size_str}{next_arg_separator}{extra_args_str}'
    return url

def query_items_all(graph_client, base_url,max_page=3,page_size=500, extra_args=None):
    ...
    logger.info(f'paging_enabled: {paging_is_enabled}, max_page: {max_page}, page_size {page_size}')
 
    graph_results=graph_generator_f(graph_client=graph_client,endpoint=url,max_page=max_page)
...
def get_az_devices(graph_client):
    api_ret_list=query_items_all(graph_client,base_url="/devices",max_page=0,page_size=500)
    logger.info(f'collected device objects {len(api_ret_list)}')
    return api_ret_list


def get_az_users(graph_client):
    api_ret_list=query_items_all(graph_client,base_url="/users",max_page=0,page_size=500)
...
def get_az_groups(graph_client,expand=False):    
    ...
    api_ret_list=query_items_all(graph_client,base_url="/groups",max_page=0,page_size=500,extra_args="$expand=members")
...
def get_intune_managed_devices(graph_client):    
    ...
    api_ret_list=query_items_all(graph_client,base_url="/deviceManagement/managedDevices",max_page=0,page_size=500)
...
def extras_into_event(sourcetype, timestamp, batch_id, value):
    ...
    logger.debug(f'trying to fix up: sourcetype: {sourcetype} batch_id: {batch_id} timestamp: {timestamp}, value: {value}')
...
def _fetch():
    ...
    graph_client_beta=GraphClient(credential=creds,api_version=APIVersion.beta,NationalClouds=NationalClouds.Global,max_retries=3, retry_backoff_factor=0.5)
    
    # for when you need endpoints that are only in beta eg intune auditEvents (log  not inventory though)
    graph_client_v1=GraphClient(credential=creds,api_version=APIVersion.v1,NationalClouds=NationalClouds.Global,max_retries=3, retry_backoff_factor=0.5)
    ...
    if(Write_File):
        write_to_json_file(intune_managed_devices,'intune_managed_devices.json',Format=True)
    
    timestamp = round(datetime.now(timezone.utc).timestamp(),0)
    batch_id = uuid.uuid4().hex
    az_devices_m = map(partial(extras_into_event,'az:graph_devices',timestamp,batch_id), az_devices)
    az_users_m =  map(partial(extras_into_event,'az:graph_users',timestamp,batch_id), az_users)
    az_groups_m =  map(partial(extras_into_event,'az:graph_groups',timestamp,batch_id), az_groups)
    intune_managed_devices_m = map(partial(extras_into_event,'intune:graph_managed_devices',timestamp,batch_id), intune_managed_devices)
    return flatten([az_devices_m,az_users_m,az_groups_m,intune_managed_devices_m])



def _process(records):
    logger.info(f'Total of {len(records)} fetched from az and intune, about to publish to splunk') 
    ...
    for batch in batches:
        publish(http, batch, splunk_token, time_field="time", sourcetype_field='sourcetype_override')  
        sum_event_count+=len(batch)
        logger.info(f'published batch number: {batch_count} to splunk, event count: {len(batch)}, total so far: {sum_event_count}, of run grand total {total_event_count}')
        batch_count+=1
